const admin = require('firebase-admin');

const clientEmail = "firebase-adminsdk-fbsvc@gravoka-7445d.iam.gserviceaccount.com";
const privateKey = "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n";
const projectId = "gravoka-7445d";

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert({
      projectId,
      clientEmail,
      privateKey,
    })
  });
}

const db = admin.firestore();

async function setExpiryDates() {
  const snapshot = await db.collection('empresas').get();
  
  const now = new Date();
  const nextMonth = new Date();
  nextMonth.setDate(now.getDate() + 30);
  
  const midMonth = new Date();
  midMonth.setDate(now.getDate() + 12);
  
  const expiryDate = admin.firestore.Timestamp.fromDate(nextMonth);
  const midDate = admin.firestore.Timestamp.fromDate(midMonth);

  console.log('Actualizando fechas de vencimiento...');
  
  let i = 0;
  for (const doc of snapshot.docs) {
    const date = i === 0 ? expiryDate : midDate;
    await doc.ref.update({ 
      fecha_vencimiento: date,
      plan_activo: doc.data().plan_activo || 'Pro'
    });
    console.log(`- ${doc.data().nombre}: Vence en ${i === 0 ? 30 : 12} días`);
    i++;
  }
  
  process.exit(0);
}

setExpiryDates();
