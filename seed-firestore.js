const admin = require('firebase-admin');

// Ensure app is not initialized multiple times if run in a strange context
if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert({
      projectId: 'gravoka-7445d',
      clientEmail: 'firebase-adminsdk-fbsvc@gravoka-7445d.iam.gserviceaccount.com',
      privateKey: '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n'
    })
  });
}

const db = admin.firestore();
db.settings({ preferRest: true });
const targetUid = 'uxpjdaPALfaqKQomcSPEJzKIpR53';

async function seedUser() {
  try {
    console.log(`Starting Firestore seed for UID: ${targetUid}`);
    
    // 1. Create the tenant company
    const empresaRef = await db.collection('empresas').add({
      nombre: 'Planeta X',
      rut: '77.888.999-0',
      plan_actual: 'pro',
      estado: 'activo',
      creado_en: admin.firestore.FieldValue.serverTimestamp()
    });
    
    console.log(`Company 'Planeta X' created with ID: ${empresaRef.id}`);

    // 2. Create the user profile linked to the company
    await db.collection('usuarios').doc(targetUid).set({
      email: 'demo@planetax.cl',
      nombre: 'Admin Planeta X',
      rol: 'admin_tenant',
      empresa_id: empresaRef.id,
      creado_en: admin.firestore.FieldValue.serverTimestamp()
    });

    console.log(`User profile for ${targetUid} linked to company ${empresaRef.id} successfully!`);
  } catch (error) {
    console.error('Error seeding user:', error);
  }
}

seedUser();
