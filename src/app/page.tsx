...
export default function Dashboard() {
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white selection:bg-primary selection:text-white font-sans overflow-x-hidden">
        ...
        <section className="relative pt-44 pb-32 px-6 flex flex-col items-center z-10">
          ...
          <h1 className="text-6xl md:text-[9.5rem] font-black tracking-tighter text-center max-w-6xl mb-12 leading-[0.85] animate-in fade-in zoom-in-95 duration-1000 delay-100 select-none uppercase">
            DESPACHO <span className="text-transparent bg-clip-text bg-gradient-to-br from-primary via-blue-400 to-emerald-400">DIGITAL</span> PARA ÁRIDOS
          </h1>
          ...
          <p className="text-lg md:text-2xl text-slate-400 text-center max-w-3xl mb-16 font-medium leading-relaxed opacity-0 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300">
            Digitaliza tus guías de despacho, registra cada viaje al instante y controla el movimiento de tu material desde cualquier lugar. La herramienta definitiva para plantas de áridos.
          </p>
        ...
        <section id="features" className="py-40 px-6 max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { icon: 'bolt', title: 'Despacho Digital', desc: 'Emisión de guías en segundos desde tu tablet o smartphone.' },
              { icon: 'analytics', title: 'Reportes Inteligentes', desc: 'Gráficos de ventas, volumen y m³ despachados actualizados al instante.' },
              { icon: 'hub', title: 'Operación Centralizada', desc: 'Gestiona múltiples clientes y faenas bajo una sola cuenta SaaS segura.' },
              { icon: 'cloud', title: 'Acceso Universal', desc: 'Sin instalaciones. Accede a tus datos históricos desde la oficina o la faena.' },
              { icon: 'monitoring', title: 'Trazabilidad Real', desc: 'Registro histórico detallado de cada viaje, material y cliente.' },
              { icon: 'local_shipping', title: 'Control de Flota', desc: 'Seguimiento de patentes, choferes y vueltas diarias.' },
              { icon: 'security', title: 'Datos Protegidos', desc: 'Tus datos están protegidos con encriptación y respaldos diarios.' },
              { icon: 'support_agent', title: 'Soporte Chileno 24/7', desc: 'Un equipo de ingenieros listo para ayudarte en cada paso.' },
            ].map((f, i) => (
          ...
          <div className="max-w-7xl mx-auto relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-5xl mx-auto">
              ...
              <div className="p-12 bg-slate-900/50 border border-white/10 rounded-[56px] backdrop-blur-3xl flex flex-col hover:border-white/20 transition-all group">
                <span className="bg-slate-800 text-slate-400 px-4 py-1.5 rounded-full text-[10px] font-black tracking-widest uppercase mb-8 w-fit">EL GANCHO</span>
                <h4 className="text-3xl font-black mb-2 uppercase tracking-tighter">STARTUP</h4>
                <div className="flex items-baseline gap-2 mb-10">
                  <span className="text-6xl font-black text-white">$0</span>
                  <span className="text-slate-500 font-bold uppercase text-[10px] tracking-widest">Siempre Gratis</span>
                </div>
                <ul className="space-y-5 mb-12 flex-1">
                  {['Guías Ilimitadas', 'Gestión de Camiones', 'Gestión de Materiales', 'Exportación Excel', '1 Planta / Sucursal'].map((item, i) => (
                    <li key={i} className="flex items-center gap-4 text-slate-400 font-bold text-xs uppercase tracking-tight">
                      <span className="material-symbols-outlined text-primary text-xl">done_all</span> {item}
                    </li>
                  ))}
                </ul>
                <Link href="/registro" className="w-full py-6 bg-white/5 border border-white/10 rounded-3xl font-black text-center text-sm hover:bg-white/10 transition-all uppercase tracking-widest">Empezar Ahora</Link>
              </div>

              {/* Full Plan */}
              <div className="p-12 bg-slate-900 relative border-2 border-primary rounded-[56px] shadow-[0_40px_100px_rgba(var(--primary-rgb),0.15)] flex flex-col scale-105 overflow-hidden">
                <div className="absolute top-0 right-10 bg-primary px-6 py-3 rounded-b-2xl text-[10px] font-black uppercase tracking-widest text-white animate-pulse">RECOMENDADO</div>
                ...
                <div className="flex items-baseline gap-2 mb-10">
                  <span className="text-6xl font-black text-white">$45k</span>
                  <span className="text-primary font-bold uppercase text-[10px] tracking-widest">/ Mes x Planta</span>
                </div>
                <ul className="space-y-5 mb-12 flex-1">
                  {['Todo lo de Startup', 'Dashboard BI Estratégico', 'Control Multi-Empresa', 'Métricas de Eficiencia', 'Personalización de Marca', 'Soporte Prioritario 24/7'].map((item, i) => (
                    <li key={i} className="flex items-center gap-4 text-slate-200 font-bold text-xs uppercase tracking-tight">
                      <span className="material-symbols-outlined text-primary text-xl">star</span> {item}
                    </li>
                  ))}
                </ul>
                <Link href="/registro" className="w-full py-6 bg-primary text-white rounded-3xl font-black text-center text-sm shadow-2xl shadow-primary/40 hover:scale-[1.02] active:scale-95 transition-all uppercase tracking-widest">Activar Plan Full</Link>
              </div>
            </div>
          </div>
        </section>
  ...
  return (
    <div className="space-y-8">
      {/* Subscription Banner */}
      {empresaData && (
        <div className={`p-3 md:p-4 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4 border ${(daysRemaining() || 0) < 7
            ? 'bg-amber-50 border-amber-200 text-amber-800'
            : 'bg-primary/5 border-primary/10 text-primary-dark font-medium'
          }`}>
          <div className="flex items-center gap-3 md:gap-4 w-full md:w-auto">
            <div className={`size-10 md:size-12 rounded-xl flex items-center justify-center ${(daysRemaining() || 0) < 7 ? 'bg-amber-100' : 'bg-primary/10'
              }`}>
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-3 py-1.5 text-xs font-medium rounded-lg ${activeChartFilter === 'Semana Actual'
